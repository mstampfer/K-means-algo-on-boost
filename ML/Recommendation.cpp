...
SimilarityPrefMapT Recommendation::calculateSimilarItems()
{
	...
	for (auto item : prefs)
	{
		...
		//Find the most similar pref to this one
		SortedPrefs scores(topMatches(
			item.first,
			bind(&Recommendation::sim_pearson, *this, _1, _2)
			));
...
python::dict Recommendation::calculateSimilarItemsWrapper()
{
	python::dict d;
	auto result = calculateSimilarItems();
	for (const auto& e : result)
	{
		d[e.first] = python::make_tuple(
...
python::dict Recommendation::getRecommendedItemsWrapper(
	const python::dict& itemMatch,
	const string &user)
{
	python::dict d;
	SimilarityPrefMapT m;
	python::list l = itemMatch.items();
	python::stl_input_iterator<python::tuple> begin(l), end;
	auto li = std::list<python::tuple>(begin, end);
	std::transform(li.begin(), li.end(), inserter(m, m.end()), [&](auto& e)
	{
		python::tuple t = python::extract<python::tuple>(e[1]);
		string key1(move(python::extract<string>(e[0])));
		string key2(move(python::extract<string>(t[1])));
		double d(move(python::extract<double>(t[0])));
		return make_pair(key1, make_pair(d, key2));
	});

	auto sortedPrefs = getRecommendedItems(m, user);
	for (const auto& e : sortedPrefs)
	{
		d[e.first] = e.second;
	}
	return d;
...
BOOST_PYTHON_MODULE(recommendation)
{
	python::class_<Recommendation>("Recommendation",
		...
		.def("calculateSimilarItems", &Recommendation::calculateSimilarItemsWrapper)
